...
export default function ProductsAdmin() {
  ...
  const DEBUG = true; // toggle this to false to disable console logs

  // Robust helper to normalize API response body
  const normalizeResponse = (res) => {
    // res may be: parsed JSON object, or { data: ... }, or { data: { data: ..., total: ... } }
    if (!res) return {};
    if (res.data && typeof res.data === "object") {
      // If res.data contains data & total (typical), return that wrapper
      if (res.data.data || typeof res.data.total !== "undefined") return res.data;
      // otherwise res.data is already the body
      return res.data;
    }
    return res;
  };

  // Fetch products (only when showProducts is true)
  const fetchProducts = async () => {
    if (!showProducts) return;
    setLoading(true);
    try {
      // Pass params as object so api wrapper can append query string
      ...
      // body may be array or { data: [...], total: N } or { products: [...], total: N }
      let list = [];
      let total = 0;
  ...
  const fetchStats = async () => {
    try {
      ...
      // body expected to include at least total & sold (or totalProducts/soldProducts)
  ...
  // Re-run products when filters change (only if products are visible)
  useEffect(() => {
    if (!showProducts) return;
    // reset to page 1 when q/limit/sortBy changes
    setPage(1);
  ...
  }, [q, page, limit, sortBy, showProducts]);

  // Optionally refetch stats whenever products panel is opened
  useEffect(() => {
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          <motion.div
            ...
            className="p-5 rounded-xl bg-white dark:bg-gray-900 shadow hover:shadow-lg transition"
          >
      ...
      <div className="flex flex-wrap gap-4">
        <button
          ...
          className="flex items-center gap-2 px-5 py-3 bg-black text-white dark:bg-white dark:text-black rounded-lg shadow hover:scale-105 transition-transform"
        >
          <Plus className="w-5 h-5" /> Add Product
        </button>
        <button
          onClick={() => setShowBulk(!showBulk)}
          className="flex items-center gap-2 px-5 py-3 bg-white dark:bg-black border border-gray-300 dark:border-gray-700 text-black dark:text-white rounded-lg shadow hover:scale-105 transition-transform"
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-black text-black dark:text-white"
              />
            </div>
            ...
            <div className="flex items-center gap-3">
              <select
                ...
                className="w-40 pl-3 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-black dark:text-white"
              >
              ...
              {/* Sort dropdown added beside per-page control */}
              <select
                value={sortBy}
                onChange={(e) => { setSortBy(e.target.value); setPage(1); }}
                className="w-52 pl-3 pr-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-black dark:text-white"
              >
          ...
          {loading ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {products.map((p) => (
                <div
                  key={p.id}
                  className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-black shadow hover:shadow-lg transition"
                >
                  <img
                    src={p.images?.split(",")[0] || "/images/placeholder.jpg"}
                    alt={p.name}
                  ...
                  <div className="mt-3 flex justify-between items-start">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        className="p-2 rounded-md hover:bg-red-100 dark:hover:bg-red-900 text-red-600"
                      >
          ...
          {totalPages > 1 && limit !== 999999 && (
            <div className="flex justify-center items-center gap-4 pt-4">
              <button
                ...
                disabled={page === 1}
                className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-700 disabled:opacity-50 hover:bg-gray-100 dark:hover:bg-gray-800"
              >
                Prev
              </button>
