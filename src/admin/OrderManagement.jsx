...
export default function OrderManagement() {
  ...
  const fetchOrderItemsForList = async (list = []) => {
    ...
    const endpoints = ["/api/admin/orders"];
  ...
  const fetchStatsForTab = async (tab = "overall") => {
    try {
      ...
      const candidate = ["/api/admin/stats"];
      let params = {};
      if (tab === "monthly") {
  ...
  const viewOrder = async (o) => {
    ...
    const candidate = [`/api/admin/orders/${o.id}`];
    ...
    try {
      const res = await tryGetMany([`/api/admin/orders`], { orderId: o.id, page: 1, limit: 1 });
      const body = normalize(res);
      let items = null;
      if (body) {
  ...
  const updateOrderStatus = async (id, status) => {
    try {
      ...
      const candidate = [`/api/admin/orders/${id}`];
      let ok = false;
