        ...
        <div className="grid" style={{ gridTemplateColumns: `repeat(${cols}, 6px)` }}>
          {grid.map((c, i) => (
            <div
              key={i}
              title={`${c} events`}
              className={`m-[1px] w-[6px] h-[6px] rounded-sm ${
                c === 0 ? "bg-white/6 dark:bg-black/10" : c < 2 ? "bg-white/40 dark:bg-black/40" : c < 4 ? "bg-white/70 dark:bg-black/70" : "bg-white dark:bg-black"
              }`}
            />
          ))}
        </div>
...
export default function UserManagement() {
  ...
  const keyMap = useMemo(() => {
    const map = { overall: "overall", monthly: "monthly", weekly: "weekly", daywise: "day" };
    return map;
  }, []);
  ...
  // stat card definitions (six tiles styled like screenshot)
  const k = range === "daywise" ? "day" : range;
  const statTiles = [
    ...
    { label: "Shipped", value: aggregated.orders_successful }, // demo mapping
    { label: "Delivered", value: aggregated.orders_successful }, // demo mapping
    ...
    <div className="min-h-screen bg-black text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="rounded-2xl p-4 border border-neutral-800 bg-neutral-900 mb-6">
          <div className="flex items-center gap-3 mb-4">
            {[
              ...
              <button
                ...
                className={`px-4 py-2 rounded-full text-sm ${range === t.key ? "bg-white text-black border border-white" : "bg-transparent border border-neutral-800 text-white/80"}`}
              >
                {t.label}
              </button>
            ))}
            ...
            <div className="ml-auto flex items-center gap-3">
              ...
              <button onClick={() => { alert("Refresh (demo)"); }} className="px-4 py-2 rounded-xl border border-neutral-700 bg-white text-black">Refresh</button>
              <button onClick={() => { alert("Export (demo)"); }} className="px-4 py-2 rounded-xl border border-neutral-700 bg-white text-black flex items-center gap-2">
          ...
          {/* KPI container */}
          <div className="rounded-xl p-4 border border-neutral-800 bg-neutral-950 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {statTiles.map((s, i) => (
              <div key={s.label} className="p-4 rounded-lg bg-neutral-900 border border-neutral-800 flex items-center justify-between">
                <div>
        ...
        {/* actions row (Browse / Update / Bulk Update) */}
        <div className="flex items-center gap-3 mb-4">
          <button className="px-4 py-2 rounded-full bg-white text-black border border-white">Browse Users</button>
          <button className="px-4 py-2 rounded-full border border-neutral-800">Update Users</button>
          <button className="px-4 py-2 rounded-full border border-neutral-800" onClick={() => startBulkAction("block")}>Bulk Update</button>
          ...
          <div className="ml-auto flex items-center gap-2">
            <div className="flex items-center bg-neutral-900 rounded-md border border-neutral-800 px-3 py-1 gap-2 w-96">
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search users (id / name / email)" className="bg-transparent outline-none text-white/90 placeholder:text-white/50 w-full" />
            </div>

            <div className="flex items-center gap-2">
              <div className="relative">
                <button className="px-3 py-2 rounded border border-neutral-800 flex items-center gap-2 text-sm">
                  {sortOrder === "newest" ? "Newest" : "Oldest"} <ChevronDown className="w-4 h-4" />
                </button>
                {/* quick sort dropdown could be inserted here */}
              </div>

              <select value={pageSize} onChange={(e) => setPageSize(Number(e.target.value))} className="px-3 py-2 bg-neutral-900 border border-neutral-800 rounded text-sm">
        ...
        <div className="rounded-2xl overflow-hidden border border-neutral-800 bg-neutral-900 shadow">
          ...
          <div className="px-4 py-3 border-t border-neutral-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => startBulkAction("block")} className="px-3 py-1 rounded border border-neutral-800">Block selected</button>
              <div className="text-sm text-white/60">Page {page} / {pageCount}</div>
              <div className="flex items-center gap-2">
                <button onClick={() => setPage((p) => Math.max(1, p - 1))} className="px-3 py-1 rounded border border-neutral-800">Prev</button>
                <button onClick={() => setPage((p) => Math.min(pageCount, p + 1))} className="px-3 py-1 rounded border border-neutral-800">Next</button>
              </div>
            </div>
          </div>
        </div>
        ...
        {openUser && (
          <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 px-4">
            ...
            <motion.div
              ...
              className="relative z-10 w-full max-w-3xl bg-neutral-950/95 border border-neutral-800 rounded-2xl p-6 shadow-xl"
            >
              <div className="flex items-start justify-between">
                <div>
                  <h3 className="text-2xl font-semibold">{openUser.name}</h3>
              ...
              {/* stats header (radio-like) */}
              <div className="flex items-center gap-3 mb-3">
                {["overall", "monthly", "weekly", "day"].map((r) => (
                  <label key={r} className={`inline-flex items-center gap-2 px-3 py-1 rounded-md border ${timeRange === r ? "bg-white text-black border-white" : "border-neutral-800 text-white/80"}`}>
                    <input type="radio" name="range" value={r} checked={timeRange === r} onChange={() => setTimeRange(r)} className="hidden" />
                    <span className={`w-3 h-3 rounded-full ${timeRange === r ? "bg-black" : "bg-white/10"}`} />
                    <span className="text-sm capitalize">{r === "day" ? "Day Wise" : r}</span>
                  </label>
                ))}
                <div className="ml-auto text-sm text-white/60">Showing: <strong className="ml-1">{timeRange === "day" ? "Day Wise" : timeRange}</strong></div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  ...
                  <div className="text-lg font-semibold mt-1">{openUser.stats[timeRange === "day" ? "day" : timeRange]?.total_orders ?? openUser.stats.overall.total_orders}</div>
                </div>
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  <div className="text-xs text-white/70">Total spend</div>
                  <div className="text-lg font-semibold mt-1">{formatCurrency(openUser.stats[timeRange === "day" ? "day" : timeRange]?.total_spend ?? openUser.stats.overall.total_spend)}</div>
                </div>
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  <div className="text-xs text-white/70">Coupon savings</div>
                  <div className="text-lg font-semibold mt-1">{formatCurrency(openUser.stats[timeRange === "day" ? "day" : timeRange]?.coupon_savings ?? openUser.stats.overall.coupon_savings)}</div>
                </div>
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  <div className="text-xs text-white/70">Successful</div>
                  <div className="text-lg font-semibold mt-1">{openUser.stats[timeRange === "day" ? "day" : timeRange]?.orders_successful ?? openUser.stats.overall.orders_successful}</div>
                </div>
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  <div className="text-xs text-white/70">Cancelled</div>
                  <div className="text-lg font-semibold mt-1">{openUser.stats[timeRange === "day" ? "day" : timeRange]?.orders_cancelled ?? openUser.stats.overall.orders_cancelled}</div>
                </div>
                <div className="p-3 bg-neutral-900 rounded-lg border border-neutral-800">
                  <div className="text-xs text-white/70">Returned</div>
                  <div className="text-lg font-semibold mt-1">{openUser.stats[timeRange === "day" ? "day" : timeRange]?.orders_returned ?? openUser.stats.overall.orders_returned}</div>
                </div>
              </div>

              {/* spend sparkline + 365 heatmap */}
              ...
              <div className="mt-6 text-right">
                <button onClick={() => setOpenUser(null)} className="px-4 py-2 rounded bg-white text-black">Close</button>
              </div>
            </motion.div>
          </div>
        )}
