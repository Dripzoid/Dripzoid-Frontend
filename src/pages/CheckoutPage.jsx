...
// Cookie-only auth version: uses credentials: "include" everywhere
import React, {
  useEffect,
  useMemo,
  useState,
...
export default function CheckoutPage() {
  ...
  const isBuyNowMode =
    location.state?.mode === "buy-now" || location.state?.fromBuyNow === true;
  const fromCartDefault =
    typeof location.state?.fromCart === "boolean"
  ...
  useEffect(() => {
    ...
    const fetchSaved = async () => {
      try {
        const aRes = await fetch(buildUrl("/api/addresses"), {
          method: "GET",
          credentials: "include",
        ...
        const pRes = await fetch(buildUrl("/api/payments"), {
          method: "GET",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
        });
  ...
  const checkoutItems = useMemo(() => {
    const incoming = Array.isArray(location.state?.items)
    ...
    return (Array.isArray(incoming) ? incoming : []).map((it, idx) => {
      ...
      const images = Array.isArray(prod.images)
        ? prod.images.join(",")
        : prod.images ?? it.images ?? it.image ?? "";
  ...
  const handleSaveAddress = async () => {
    if (!isShippingValid()) {
      return alert("Please fill required shipping fields before saving.");
    }
    ...
    if (!payload.line1 || !payload.city || !payload.state || !payload.pincode) {
      return alert(
        "To save to your account please ensure Address (line1), City, State and Pincode are filled."
      );
    }

    try {
      const res = await fetch(buildUrl("/api/addresses"), {
        method: "POST",
        credentials: "include",
        headers: {
  ...
  const handleDeleteAddress = async (addrId) => {
    ...
    try {
      const res = await fetch(buildUrl(`/api/addresses/${addrId}`), {
        method: "DELETE",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  async function redeemCoupon(code) {
    ...
    if (!user) return alert("Please login to redeem coupons.");
    ...
    try {
      ...
      const res = await fetch(buildUrl("/api/coupons/redeem"), {
        method: "POST",
        credentials: "include",
        headers: {
          "Content-Type": "application/json",
  ...
  const createRazorpayOrderOnServer = async (orderPayload) => {
    const res = await fetch(buildUrl("/api/payments/razorpay/create-order"), {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
  ...
  const verifyRazorpayPayment = async (verifyPayload) => {
    const res = await fetch(buildUrl("/api/payments/razorpay/verify"), {
      method: "POST",
      credentials: "include",
      headers: {
        "Content-Type": "application/json",
  ...
  const handlePayment = async ({ fromCart = fromCartDefault } = {}) => {
    ...
    try {
      ...
      const placeOrderPayload = {
        ...
        paymentDetails:
          paymentType === "cod" ? { method: "COD" } : { method: "Razorpay" },
      ...
      const createOrderPayloadForRazor = {
        ...
        shipping: {
          ...
          customerName: shippingNormalized.customerName,
      ...
      if (paymentType === "cod") {
        const res = await fetch(buildUrl("/api/orders/place-order"), {
          method: "POST",
          credentials: "include",
          headers: {
            "Content-Type": "application/json",
      ...
      const options = {
        ...
        order_id: rOrderId,
        prefill: {
          name: shippingNormalized.customerName || user?.name || "",
          email: user?.email || "",
          contact: shippingNormalized.phone || "",
        },
        ...
        <div className="text-sm">
          <div
            className={`font-medium ${
              active ? "text-neutral-900 dark:text-white" : "text-gray-500 dark:text-gray-300"
            }`}
          >
            {label}
          </div>
  ...
  const inputBase =
    "w-full rounded-lg border px-3 py-2 focus:outline-none focus:ring-2 focus:ring-offset-1 transition";
  const inputLight = "bg-white border-gray-200 text-gray-900 focus:ring-indigo-500";
  const inputDark =
    "dark:bg-gray-800 dark:border-gray-700 dark:text-gray-100 dark:focus:ring-indigo-400";
  ...
  const selectedBtnClass = "shadow-lg";
  const unselectedBtnClass = "bg-white dark:bg-gray-900";
  const selectedCommon = "bg-neutral-900 text-white dark:bg-white dark:text-black";
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden sm:flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
            ...
            <span className="ml-1 font-medium">{shipping.city || "Select address"}</span>
          </div>
          <div className="hidden sm:flex items-center gap-2 text-sm text-gray-600 dark:text-gray-300">
            <UserIcon size={16} />{" "}
              ...
              <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="rounded-lg border p-4 bg-gray-50 dark:bg-gray-800">
                  ...
                  {promoApplied ? (
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                      Have a coupon? Apply it to get instant discount.
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-lg border p-4 bg-gray-50 dark:bg-gray-800">
                  ...
                  <div className="border-t mt-3 pt-3 flex justify-between text-base font-bold">
                    <div>Total</div>
                    <div>₹{fmt(itemsTotal + shippingCost - (promoApplied?.amount ?? 0))}</div>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  {savedAddresses.map((addr) => (
                    <div
                      ...
                      className={`p-4 rounded-xl border ${
                        addr.id === shipping.id
                          ? "border-neutral-900 dark:border-neutral-600 bg-neutral-50 dark:bg-gray-800"
          ...
          {step === 3 && (
            <div className={`${cardBase} ${cardLight} ${cardDark} p-6`}>
              ...
              <div className="mt-4 text-xs text-gray-500">
                Secure payment — Razorpay handles sensitive card/UPI flows. COD available as fallback.
              </div>
            </div>
          )}
        </div>
              ...
              <div className="p-5 bg-gradient-to-r from-neutral-900 to-neutral-700 text-white">
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-white/80">
                    {checkoutItems.reduce((s, it) => s + it.quantity, 0)} items
                  </div>
                </div>
              </div>
                    ...
                    {step < 3 ? (
                      ...
                      >
                        ...
                        <span className="label">
                          {loading ? "Processing..." : `Place order ₹${fmt(grandTotal)}`}
                        </span>
                      </motion.button>
                    )}
                  </div>
